...
void ControlPanelExtendAStar::setupLayout() {
    ...
    radioBtnManhattan = new QRadioButton("曼哈顿", this);
    radioBtnEuclidean = new QRadioButton("欧几里得", this);
    ...
    QGroupBox* btnGroupBox1 = new QGroupBox("", this);
...
void ControlPanelExtendAStar::onHeuristicTypeChanged(int id) {
    ...
    if (astar) {
        if (id == 0) {
            ...
            emit sigLogInfo("距离算法切换至 ‘曼哈顿’");
        } else {
            astar->setEuclideanHeuristicFunction();
            emit sigLogInfo("距离算法切换至 ‘欧几里得’");
        }
    }
}
